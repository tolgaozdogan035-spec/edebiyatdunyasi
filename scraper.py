...
PINNED_INTERVIEW = {
    ...
    "desc": (
        "Fuarcılık sektöründeki yöneticilik kariyerini edebiyat dünyasındaki"
        " derin gözlemleriyle harmanlayan yazar Tolga Özdoğan ile yazarlık"
        " serüveni ve 'Leyla - Yasak Mevsim' üzerine çok özel bir söyleşi."
    ),
    "content": """
...
def scrape_full_article(url, fallback_html, fallback_image):
  ...
  if (
    ...
    if (
        len(
            BeautifulSoup(scraped_html, "html.parser").get_text(strip=True)
        )
        > 200
    ):
...
def clean_turkish_content(html_content, source_name):
  ...
  for a in soup.find_all("a"):
    if (
        "devam" in a.get_text().lower()
        or "read more" in a.get_text().lower()
    ):
      a.decompose()
    else:
      a.unwrap()
  ...
  return (
      valid_html
      + f"<br><hr><br><p><b>Kaynak Bilgisi:</b> Bu içerik {source_name} üzerinden"
      " derlenmiştir.</p>"
  )
...
def save_to_google_drive(json_str, file_name):
  try:
    ...
    results = (
        service.files()
        .list(q=f"name='{file_name}' and trashed=false", spaces="drive", fields="files(id, name)")
        .execute()
    )
    ...
    if items:
      ...
      service.files().create(
          body={"name": file_name, "mimeType": "application/json"},
          media_body=media,
      ).execute()
  except:
    pass
  ...
  for source in ALL_SOURCES:
    ...
    for entry in getattr(feed, "entries", [])[:entry_limit]:
      try:
        ...
        is_interview = False
        if any(
            w in title.lower() or w in link.lower()
            for w in ["röportaj", "söyleşi", "mülakat"]
        ):
          is_interview = True
        ...
        raw_text_check = BeautifulSoup(
            raw_content, "html.parser"
        ).get_text(strip=True)
        if (
            len(raw_text_check) < 450
            or raw_text_check.endswith("...")
            or raw_text_check.endswith("…")
            or "[&" in raw_content
            or "unsplash.com" in final_image
        ):
          if link:
            raw_content, final_image = scrape_full_article(
                link, raw_content, final_image
            )

        final_content = clean_turkish_content(raw_content, source["name"])

        plain_desc = (
            BeautifulSoup(final_content, "html.parser").get_text()[:200] + "..."
        )
        ...
        if not raw_date:
          raw_date = (
              entry.get("updated")
              if isinstance(entry, dict)
              else getattr(entry, "updated", None)
          )
...
def fetch_tolga_articles():
  ...
  try:
    ...
    if res.status_code == 200:
      ...
      for h in soup.find_all(["h2", "h3"]):
        ...
        if len(title) < 15 or "Tolga Özdoğan" in title:
          continue
        ...
        if link_tag:
          ...
          link = (
              ...
              else "https://tolgaozdogan.com/" + l.lstrip("/")
          )

        date = "Güncel"
        date_tag = parent.find(
            lambda tag: tag.name in ["span", "time", "p"]
            and any(
                m in tag.get_text().lower()
                for m in [
                    "ağustos",
                    "eylül",
                    "ekim",
  ...
  if not yazilar:
    yazilar = [{
        "title": (
            "GÖRÜNMEYENİN LABİRENTİNDE İNSAN: AĞUSTOS 2026 ÇOK SATANLARINDA"
            " HAKİKAT VE YANILSAMA"
        ),
        "link": "https://tolgaozdogan.com/kose-yazilari.html",
        "date": "28 Ağustos 2026",
        "desc": (
            "Ağustos ayının son günlerinde hem dünya genelinde hem de Türkiye"
            " raflarında çok satanlar listelerini incelediğimde, edebiyatın"
            " kolektif bilincimizde açtığı yeni bir damar dikkatimi çekiyor."
            " Riley Sager'ın küresel ölçekte listeleri altüst eden The"
            " Unknown romanından Virginia Evans'ın sarsıcı çıkış eseri"
            " Muhabbet'e, Rachel Cusk'ın merakla beklenen Life of M'inden Alice"
            " Feeney'nin psikolojik gerilimlerine..."
        ),
    }]
  return yazilar
...
if __name__ == "__main__":
  ...
  edebiyat_odulleri = scrape_edebiyat_odulleri()
  ...
  try:
    ...
    save_to_google_drive(
        json.dumps(interviews, ensure_ascii=False, indent=4),
        "edebiyat_gundemi_soylesiler.json",
    )
  except:
    pass

  try:
    with open("haberler/tolga_yazilari.json", "w", encoding="utf-8") as f:
      json.dump(tolga_yazilari, f, ensure_ascii=False, indent=4)
    save_to_google_drive(
        json.dumps(tolga_yazilari, ensure_ascii=False, indent=4),
        "tolga_ozdogan_yazilari.json",
    )
  except:
    pass

  try:
    with open("haberler/edebiyat_odulleri.json", "w", encoding="utf-8") as f:
      json.dump(edebiyat_odulleri, f, ensure_ascii=False, indent=4)
    save_to_google_drive(
        json.dumps(edebiyat_odulleri, ensure_ascii=False, indent=4),
        "edebiyat_odulleri.json",
    )
    print(
        "Başarılı: Edebiyat ödülleri oluşturuldu"
        f" ({len(edebiyat_odulleri)} adet)."
    )
  except:
    pass
